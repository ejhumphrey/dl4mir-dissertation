...
However, Western ``pop music'', infused with elements of folk, blues, jazz, rock and countless other influences, is not to required to adhere to or consider the rules of traditional tonal theory.
...
An alternative approach to analysis, such as voice leading, might make more sense in this instance; in others, such as rap or ``math rock'', a lack of clearly structured harmonic content may arguably render the goal of harmonic analysis irrelevant.
...
R_{micro} = \frac{1}{S}\sum_{n=0}^{N-1}\int_{t=0}^{T_n}C(\mathcal{R}_n(t), \mathcal{E}_n(t))\partial~t
\end{equation}
...
However, given the significant imbalance of chord classes, large swings in the overall micro-recall statistic may result in small differences of this qualitywise macro-recall, and vice versa.
...
Additionally, the application of local-contrast normalization is considered as an experimental variable.
...
These configurations are given in Table \ref{tab:model_configs}, and an index tuple notation is adopted to compactly reference the different models.
...
The output of the final layer is passed through a softmax operator, producing an output that behaves like a probability mass function over the chord classes.
...
Following the discussion of evaluation in \ref{subsec:eval_methodology}, the only comparison function used here is ``thirds'', and all statistics given here correspond to the micro-recall measure.
...
An instance of one such ``problem'' track is ``With or Without You'' by U2.
...
Additionally, there is some evidence, in the vein of Figure \ref{fig:acc_diff}, that chord with modified intervals or bass information may themselves be a source of noise in the reference chord annotations.
