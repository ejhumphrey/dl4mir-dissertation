...
For example, there are slight discrepancies in root movement at the end of the sequence, where DT expands the \textt{A:maj} of TdC and DNN into \textt{A:maj}-\textt{F\#:min}-\textt{C\#:min}, or relatively, I-vi-iii.
This motion occurs multiple times in the song, and each perspective is consistent in its interpretation.
...
DT says major, TdC says dominant 7, deep network says both, but only the dominant seven when the flat-seventh is played by a string part (27 seconds in).
This causes considerable mismatch between annotators and, to an extent, in evaluating the automatic system, because everyone is in some sense correct.
...
A4 is approximately 426Hz, putting the song roughly a quartertone flat.
