...
Automatic chord estimation (ACE), one of the oldest subtopics in the field of content-based MIR, embodies the opposite scenario, being rather ill-defined and based on subjective information almost exclusively.
Adopting a similar approach, a deep convolutional neural network is used to estimate the likelihoods of various chord classes from time-frequency representations of audio.
In addition to the standard objective of advancing the state of the art in a chosen application domain, this chapter seeks to explore the limitations of deep learning as applied to a complex, subjective music intelligence task.
...
Pitch is defined as the perceptual phenomena whereby a sound stimulus can be matched to the frequency of a pure sinusoid, known as the fundamental frequency \cite{}.
...
A scale's \emph{degrees}, or distance in semitones from the starting note, are expressed by the following:
...
These sequences in (\ref{eq:scales}) can be used to recover a scale by circularly indexing the set of pitch classes given in (\ref{eq:pitch_classes}).
...
Comprised of three notes, a triad is built by taking the second and fourth scale degrees from a chosen starting point, referred to as the \emph{root}.
...
Accordingly, a ``key'' takes a pitch class and either a major or minor quality, e.g. $E-major$ or $C\sharp-minor$.
Key is integral to the discussion here, because its impression or expectation establishes a harmonic framework with which one can parse music, leading to the understanding of notes and intervals as they relate to scales and chords.
...
In practice, a chord is named based on three, often subjective, criteria: its root, its contributing intervals, and how these two relate to the key.
...
A last example, shown in Figure \ref{fig:mthomework}, illustrates the level of complexity and inherent subjectivity that may arise in the process of describing music in the language of chords.
...
There are myriad observations that might be drawn from this example, but a few are of particular interest here.
...
As notated in measure 2 for example, the combination of the $A$ and $F$ can be understood both as an embellishment on the $I$, or as a separate passing $iii$ chord.
When performed, however, the musician can influence how one might percieve this simultaneity through the use of expressive timing or dynamics.
...
An example of this might be the chord name \texttt{A:min(\*b3, b7)}, indicating that the minor third ($C$) is absent and a minor 7 ($G$) has been added.
...
The final field of this chord syntax is the bass interval, $b$, which indicates the scale degree of the lowest contributing pitch.
...
Western ``pop music'', however, is not wont to consider the rules of conventional tonal theory or compose by traditional means, infused with elements of folk, blues, jazz, rock and countless other genres.
Therefore efforts to understand the former in the language of the latter is ultimately limited by the validity in doing so.
...
An alternative approach to analysis, such as voice leading, might make more sense in this instance; in others, such as rap or ``math rock'', a lack of clearly structured harmonic content may arguably render the goal of harmonic analysis altogether irrelevant.
As genre is itself an amorphous and poorly defined concept, the degree to which a piece of music might be understood harmonically will vary, both absolutely and internally.
...
As discussed in \ref{subsec:syntax}, it is a particular nuance of chord notation that the space of valid spellings is effectively infinite.
...
This simplification reduces the task of chord estimation to a classification problem, where all inputs are assigned to one of $K$ chord \emph{classes}.
...
To address this challenge, the common strategy employed by the research community is that of Major-Minor chord resolution.
...
% How do we know when we've got it -- comparing the chords it produced with the chords we have
Having framed chord estimation as a flat classification problem, there are two critical assumptions to note going forward.
First, the classification paradigm operates on the notion that the relationship between an observation and its corresponding class is absolute.
Chord estimation research has classically leveraged expert musicians to approximately achieve objectivity.
Additionally, any input can belong to at most one class, therefore assuming separability between classes.
...
\begin{figure}[!t]
\centering
 \centerline{\includegraphics[width=\textwidth]{tr-te-diff_FF-TT}}
