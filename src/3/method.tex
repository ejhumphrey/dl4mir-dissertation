...
The \emph{maximum likelihood}, or ML, decision rule is a simple, and thus attractive, one in this case, which identifies the maximum likelihood as the winning class index:

\begin{equation}
\label{eq:map}
\hat{\Theta}_{\mathrm{ML}}(x)
= \underset{\theta}{\operatorname{arg ~ max}} \ f(x | \Theta)
\end{equation}

\noindent To leverage this formulation, it is necessary to make the network's output behave like a posterior likelihood estimator, i.e. non-negative and sums to 1.
This is traditionally achieved in deep networks by flattening the output of an architecture and passing it through the \emph{softmax} function, defined as follows:
...
% Radial Basis
The other common approach to classification in neural networks is with a Radial Basis function, i.e. the $L_2$ distance to a collection of class centroids:

\begin{equation}
f(x) = \sum(x - \mathbf{w})^2
\end{equation}

Equivalent to template matching or nearest centroid classification, the advantage of incorporating the radial basis function \emph{in} a network is two-fold:
first, parameters can be optimized to a set of chosen centroid, which may have semantic importance, as in \cite{LeCun1998};
and second, these class centroids could be learned in the process of numerical optimzation.
