...
Notably, local receptive fields can be realized as an affine transformation against a mostly-sparse set of weights, expressed as the following:
...
\subsubsection{Other Common Operations}

% Pooling
the parameter $p_0, p_1$ is a two-element tuple that defines the neighborhood of the max operator along the feature dimensions.
Max pooling is the most common form, usually applied to a local neighborhood.
There is also the L2 variant
Relationship to max-out networks, difference here being that pooling is typically exploiting spatial correlations.
